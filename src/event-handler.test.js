...
describe('<EventHandler />', () => {
  ...
  it('unregisters handlers from target on unmount', () => {
    const change = jest.fn();
    renderer.create(<EventHandler target={target} change={change} />).unmount();
  ...
  it('unregisters handlers from target when target changes', () => {
    ...
    renderer
      .create(<EventHandler target={target} change={change} />)
      .update(<EventHandler target={target2} change={change} />);
  ...
  it('adds new handlers', () => {
    ...
    renderer
      .create(<EventHandler target={target} change1={change1} />)
      .update(
  ...
  it('removes unused handlers', () => {
    ...
    renderer
      ...
      .update(<EventHandler target={target} change1={change1} />);
    target.dispatchEvent(evt1);
    target.dispatchEvent(evt2);
  ...
  it('calls updated handler without re-registering new one on target', () => {
    ...
    renderer
      .create(<EventHandler target={target} change={() => change(1)} />)
      .update(<EventHandler target={target} change={() => change(2)} />);
    target.changed();
